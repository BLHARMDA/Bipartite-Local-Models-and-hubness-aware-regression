...
@numba.jit
def BLHARMDA(F_M,F_D,k,N,M,D): #k:the k in kNN N:repeated times
    ...
    # reoeated for N times
    for time in range(N):
        
        # randomly choose F_M,F_D columns
        j_m=random.sample(range(2*M_num),F_M)
        j_d=random.sample(range(2*D_num),F_D)
        a_m=np.ones(F_M)
        a_d=np.ones(F_D)
        i_m=np.zeros(F_M)
        i_d=np.zeros(F_D)
        rd_m=sp.sparse.coo_matrix((a_m,[i_m,j_m]),shape=(1,2*M_num)).toarray()
        rd_d=sp.sparse.coo_matrix((a_d,[i_d,j_d]),shape=(1,2*D_num)).toarray()
        M_rd=M*rd_m
        D_rd=D*rd_d
...
def scoresNranks(F_M=495*2,F_D=383*2,k=100,N=1):
...
''' RUN '''
def __run__(F_M,F_D,k,N):
    __start__()
    load4cs()
    print 'data loaded\n'
    scoresNranks(F_M,F_D,k,N)
    case1_test()
    print 'done\n'
